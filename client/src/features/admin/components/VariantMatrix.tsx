...
export const VariantMatrix: React.FC<VariantMatrixProps> = ({ productId, productCode }) => {
  ...
  return (
    <div className="bg-[#F7F7F5] rounded-xl p-6 shadow-sm border border-gray-100 max-w-5xl mx-auto my-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-gray-200 pb-4 mb-6">
        <div>
          <h2 className="text-2xl font-semibold text-[#3F3F3F]">Matriz de Variantes SKU</h2>
          <p className="text-sm text-[#6B6B6B] mt-1">
            Código Base: <span className="font-mono bg-gray-200 px-2 py-0.5 rounded text-xs">{productCode}</span>
          </p>
        </div>
        <span className="mt-2 md:mt-0 text-xs font-medium text-[#6B6B6B] bg-[#D9D9D2] px-3 py-1 rounded-full">
      ...
      {/* Generator Section */}
      {variants.length === 0 && (
        <form onSubmit={handleGenerate} className="bg-white rounded-lg p-6 border border-gray-200 mb-8">
          <h3 className="text-base font-medium text-[#3F3F3F] mb-4">Generación Inicial Masiva</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-xs font-semibold text-[#6B6B6B] uppercase mb-1">Tallas (Separar con comas)</label>
              <input
                type="text"
                value={tallasInput}
                onChange={(e) => setTallasInput(e.target.value)}
                placeholder="S, M, L"
                className="w-full text-sm border border-gray-300 rounded px-3 py-2 focus:ring-1 focus:ring-[#3F3F3F] focus:border-[#3F3F3F] outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#6B6B6B] uppercase mb-1">Colores (Separar con comas)</label>
              <input
                type="text"
                value={coloresInput}
            ...
            <div>
              <label className="block text-xs font-semibold text-[#6B6B6B] uppercase mb-1">Precio Base ($)</label>
              <input
                type="number"
                step="0.01"
                value={basePrice}
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-5 w-full bg-[#3F3F3F] hover:bg-[#2F2F2F] text-white text-sm font-medium py-2.5 rounded transition duration-200 disabled:opacity-50"
          >
      ...
      {loading && variants.length === 0 ? (
        <div className="text-center py-12 text-[#6B6B6B] text-sm font-medium">Cargando variantes...</div>
      ) : variants.length > 0 ? (
        <div className="overflow-x-auto bg-white rounded-lg border border-gray-200 shadow-sm">
          <table className="min-w-full divide-y divide-gray-200 text-left text-sm">
            <thead className="bg-[#D9D9D2]/20 text-[#3F3F3F] font-semibold text-xs uppercase tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-gray-100 text-[#6B6B6B]">
              {variants.map((v) => {
                  ...
                  <tr key={v.id} className="hover:bg-gray-50 transition duration-150">
                    ...
                    <td className="px-6 py-4 font-mono text-xs">
                      {isEditing ? (
                        <input
                          ...
                          className="border border-gray-300 rounded px-2 py-1 focus:ring-1 focus:ring-[#3F3F3F] outline-none text-xs w-48"
                        />
                      ) : (
                        v.sku
                      )}
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900">
                      {isEditing ? (
                        <input
                          ...
                          className="border border-gray-300 rounded px-2 py-1 focus:ring-1 focus:ring-[#3F3F3F] outline-none text-xs w-24"
                        />
                      ) : (
                        `$${v.price.toFixed(2)}`
                      )}
                    ...
                    <td className="px-6 py-4 text-right">
                      {isEditing ? (
                        ...
                        <button
                          onClick={() => startEdit(v)}
                          className="text-xs font-semibold text-[#3F3F3F] hover:text-black hover:underline"
                        >
                          Editar
                        </button>
                      )}
